...
function normalizeExactNumber(numerator: bigint, denominator: bigint): ExactNumber {
    if (denominator === 0n) throw new RangeError("Division by zero is not allowed.");
...
function floorExactDivision(numerator: bigint, denominator: bigint): bigint {
    const quotient = numerator / denominator;
    const remainder = numerator % denominator;
    return remainder !== 0n && numerator < 0n ? quotient - 1n : quotient;
}

function ceilExactDivision(numerator: bigint, denominator: bigint): bigint {
    const quotient = numerator / denominator;
    const remainder = numerator % denominator;
    return remainder !== 0n && numerator > 0n ? quotient + 1n : quotient;
}

function scaleExactBoundary(value: number, scale: bigint, round: "up" | "down"): bigint {
...
function toSafeInteger(value: bigint, errorMessage: string): number {
    const converted = Number(value);
    if (!Number.isSafeInteger(converted)) throw new RangeError(errorMessage);
    return converted;
}

function hasReasonableDigitCount(...parts: string[]): boolean {
...
export function parseExactNumber(input: string): ExactNumber | null {
    ...
    if (normalized.includes("/")) {
        ...
        if (parts.length === 1) {
            ...
            if (fractionMatch === null || !hasReasonableDigitCount(fractionMatch[1], fractionMatch[2])) return null;
    ...
    if (!hasReasonableDigitCount(integerDigits, fractionDigits, exponentText)) return null;
    ...
    if (decimalScale > 0) denominator = 10n ** BigInt(decimalScale);
    else if (decimalScale < 0) numerator *= 10n ** BigInt(-decimalScale);
    if (decimalMatch[1] === "-") numerator = -numerator;
    return normalizeExactNumber(numerator, denominator);
}

function addExactNumbers(left: ExactNumber, right: ExactNumber): ExactNumber {
    ...
    return normalizeExactNumber(
        left.numerator * leftScale + right.numerator * rightScale,
        left.denominator * leftScale
    );
}

function multiplyExactNumbers(left: ExactNumber, right: ExactNumber): ExactNumber {
...
export function calculateExactResult(
    leftInput: string,
    rightInput: string,
    ...
    switch (operatorType) {
        ...
        case "division":
            if (right.numerator === 0n) throw new RangeError("Division by zero is not allowed.");
...
export function formatExactFraction(value: ExactNumber): string {
    ...
    if (whole === 0n) return `${sign}${remainder}/${normalized.denominator}`;
...
export function formatExactDecimal(value: ExactNumber): string | null {
    const normalized = normalizeExactNumber(value.numerator, value.denominator);
    ...
    if (remainingDenominator !== 1n) return null;
    ...
    const sign = normalized.numerator < 0n ? "-" : "";
    if (decimalPlaces === 0) return `${sign}${scaledNumerator}`;
...
export function formatExactResult(value: ExactNumber, numberType: NumberType): string {
...
function formatScaledDecimal(scaledValue: number, decimalPlaces: number): string {
    const sign = scaledValue < 0 ? "-" : "";
    const absoluteDigits = Math.abs(scaledValue).toString();
    if (decimalPlaces === 0) return `${sign}${absoluteDigits}`;
